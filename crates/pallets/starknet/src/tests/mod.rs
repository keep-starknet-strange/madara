...
use starknet_api::transaction::{
    Calldata, ContractAddressSalt, DeclareTransaction, DeclareTransactionV0V1, DeployAccountTransactionV1, Fee,
...
pub fn get_invoke_dummy(nonce: Nonce) -> InvokeTransactionV1 {
    ...
    InvokeTransactionV1 { max_fee: MAX_FEE, signature, nonce, sender_address, calldata }
}
...
fn get_invoke_argent_dummy() -> InvokeTransactionV1 {
    ...
    InvokeTransactionV1 { max_fee: MAX_FEE, signature: TransactionSignature(vec![]), nonce, sender_address, calldata }
}
...
/// with starkli.
pub fn get_declare_dummy(chain_id: Felt252Wrapper, nonce: Nonce, account_type: AccountType) -> DeclareTransaction {
    ...
    let mut tx = DeclareTransaction::V1(DeclareTransactionV0V1 {
        max_fee: MAX_FEE,
    ...
    match tx {
        DeclareTransaction::V1(mut tx) => tx.signature = signature,
        _ => unreachable!(),
    }
...
pub fn set_infinite_tokens<T: Config>(contract_address: &ContractAddress) {
    ...
    let balance_key_low = get_fee_token_var_address(fee_token_addresses.eth_fee_token_address);
