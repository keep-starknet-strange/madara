...
type UncheckedExtrinsic = frame_system::mocking::MockUncheckedExtrinsic<MockWithoutNonceValidationRuntime>;
type Block = frame_system::mocking::MockBlock<MockWithoutNonceValidationRuntime>;

// Configure a mock runtime to test the pallet.
frame_support::construct_runtime!(
    pub enum MockWithoutNonceValidationRuntime where
        Block = Block,
        NodeBlock = Block,
...
impl pallet_timestamp::Config for MockWithoutNonceValidationRuntime {
...
impl system::Config for MockWithoutNonceValidationRuntime {
...
parameter_types! {
    ...
    pub const EnableNonceValidation: bool = false;
    pub const ProtocolVersion: u8 = 0;
}

impl pallet_starknet::Config for MockWithoutNonceValidationRuntime {
    type RuntimeEvent = RuntimeEvent;
...
pub fn new_test_ext_without_nonce_validation() -> sp_io::TestExternalities {
    let mut t = frame_system::GenesisConfig::default().build_storage::<MockWithoutNonceValidationRuntime>().unwrap();
    ...
    pallet_starknet::GenesisConfig::<MockWithoutNonceValidationRuntime> {
        contracts: vec![
...
pub(crate) fn run_to_block_without_nonce_validation<T: Config>(n: u64) {
...
pub(crate) fn basic_test_setup_without_nonce_validation<T: Config>(n: u64) {
    SeqAddrUpdate::<T>::put(true);
    ...
    run_to_block_without_nonce_validation::<T>(n);
}
