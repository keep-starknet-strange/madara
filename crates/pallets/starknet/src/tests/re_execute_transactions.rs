...
fn re_execute_tx_ok() {
    new_test_ext::<MockRuntime>().execute_with(|| {
        ...
        let res =
            Starknet::re_execute_transactions(txs_to_ignore, txs.clone(), false, false).unwrap().unwrap().execution_infos;
...
fn re_execute_tx_with_a_transfer_ok() {
    new_test_ext::<MockRuntime>().execute_with(|| {
        ...
        // Call the function we want to test
        let res = Starknet::re_execute_transactions(txs.clone(), vec![transfer_tx.clone()], false, false)
            .unwrap()
            .unwrap()
            .execution_infos;
