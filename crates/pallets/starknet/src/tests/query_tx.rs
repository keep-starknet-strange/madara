...
fn estimates_tx_fee_with_query_version() {
    new_test_ext::<MockRuntime>().execute_with(|| {
        ...
        let tx = AccountTransaction::Invoke(transaction.into());
...
fn executable_tx_should_be_estimable_and_executable() {
    new_test_ext::<MockRuntime>().execute_with(|| {
        ...
        let tx_vec = vec![AccountTransaction::Invoke(transaction.clone().into())];

        // it should be valid for estimate calls
        assert_ok!(Starknet::estimate_fee(tx_vec));

        // it should be executable
        assert_ok!(Starknet::invoke(RuntimeOrigin::none(), transaction.clone().into()));
    });
}
