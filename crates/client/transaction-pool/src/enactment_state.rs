...
mod enactment_state_tests {
    ...
    fn tree_route(from: Hash, to: Hash) -> Result<TreeRoute<Block>, String> {
        ...
        let from = chain.iter().position(|bn| bn.hash == from).ok_or("existing block should be given")?;
        let to = chain.iter().position(|bn| bn.hash == to).ok_or("existing block should be given")?;
        ...
        let pivot = if from <= pivot && to <= pivot {
            if from < to { to - from } else { 0 }
        } else if from >= pivot && to >= pivot {
            if from < to { 0 } else { from - to }
        } else {
            if from < to { pivot - from } else { from - pivot }
        };
    ...
    mod mock_tree_route_tests {
        ...
        fn assert_treeroute_eq(expected: Result<TreeRoute<Block>, String>, result: Result<TreeRoute<Block>, String>) {
            ...
            assert!(result.enacted().iter().zip(expected.enacted().iter()).all(|(a, b)| a.hash == b.hash));
            assert!(result.retracted().iter().zip(expected.retracted().iter()).all(|(a, b)| a.hash == b.hash));
        }
        ...
        fn tree_route_mock_test_09() {
            ...
            let expected = TreeRoute::new(vec![e2(), d2(), c2(), b2(), a(), b1(), c1(), d1(), e1()], 4);
            assert_treeroute_eq(result, expected);
        }
        ...
        fn tree_route_mock_test_10() {
            ...
            let expected = TreeRoute::new(vec![e1(), d1(), c1(), b1(), a(), b2(), c2(), d2(), e2()], 4);
            assert_treeroute_eq(result, expected);
        }
        #[test]
    ...
    fn trigger_new_best_block(
        ...
        state
            .update(
                &ChainEvent::NewBestBlock { hash: acted_on, tree_route: Some(Arc::new(event_tree_route)) },
                &tree_route,
    ...
    fn trigger_finalized(
        ...
        let v =
            tree_route(from, acted_on).expect("Tree route exists").enacted().iter().map(|h| h.hash).collect::<Vec<_>>();
