    ...
    for (validation_result, ext_hash, ext) in validation_results {
        match validation_result {
            Ok(Err(TransactionValidityError::Invalid(err))) => {
                log::debug!(target: LOG_TARGET, "[{:?}]: Revalidation: invalid {:?}", ext_hash, err,);
            ...
            Ok(Err(TransactionValidityError::Unknown(err))) => {
                ...
                log::trace!(target: LOG_TARGET, "[{:?}]: Unknown during revalidation: {:?}", ext_hash, err,);
            }
            Ok(Ok(validity)) => {
                revalidated.insert(
                    ext_hash,
...
impl<Api: ChainApi> RevalidationWorker<Api> {
    ...
    fn push(&mut self, worker_payload: WorkerPayload<Api>) {
        ...
        for ext_hash in transactions {
            ...
            if self.members.contains_key(&ext_hash) {
                log::trace!(target: LOG_TARGET, "[{:?}] Skipped adding for revalidation: Already there.", ext_hash,);
    ...
    pub async fn run(mut self, from_queue: TracingUnboundedReceiver<WorkerPayload<Api>>, interval: Duration) {
