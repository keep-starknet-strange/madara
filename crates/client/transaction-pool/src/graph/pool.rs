...
mod tests {
    ...
    mod listener {
        ...
        fn should_trigger_ready_and_finalized() {
            ...
            assert_eq!(stream.next(), Some(TransactionStatus::InBlock((H256::from_low_u64_be(2).into(), 0))),);
        }
        ...
        fn should_trigger_ready_and_finalized_when_pruning_via_hash() {
            ...
            // when
            block_on(pool.prune_tags(&BlockId::Number(2), vec![vec![0u8]], vec![*watcher.hash()])).unwrap();
            assert_eq!(pool.validated_pool().status().ready, 0);
            assert_eq!(pool.validated_pool().status().future, 0);
        ...
        fn should_trigger_dropped_lower_priority() {
            {
                ...
                assert!(matches!(result, Err(sc_transaction_pool_api::error::Error::ImmediatelyDropped)));
            }
            {
                // given
