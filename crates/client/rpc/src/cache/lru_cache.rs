...
impl<K: Eq + core::hash::Hash, V: Encode> TraitInnerCache<K, V> for LruMap<K, V, Unlimited> {
    fn get(&mut self, k: &K) -> Option<&mut V> {
        self.get(k)
    }
...
impl<K: Eq + core::hash::Hash, V: Encode> TraitInnerCache<K, V> for LruMap<K, V, ByMemoryUsage> {
    fn get(&mut self, k: &K) -> Option<&mut V> {
        self.get(k)
    }
...
impl<K: Eq + core::hash::Hash + 'static, V: Encode + 'static> LRUCache<K, V> {
    ...
    pub fn new(
        ...
        Self {
            ...
            inner: if let Some(max_allocated_size) = max_allocated_size {
                Box::new(LruMap::new(ByMemoryUsage::new(max_allocated_size)))
            } else {
                Box::new(LruMap::new(Unlimited))
            },
            max_allocated_size,
            max_values_size,
            values_size: 0,
