...
fn known_error_code_deserialization() {
    ...
    for (code_str, known_code) in [
        ...
        ("StarknetErrorCode.CONTRACT_BYTECODE_SIZE_TOO_LARGE", KnownStarknetErrorCode::ContractBytecodeSizeTooLarge),
        (
        ...
        ("StarknetErrorCode.ENTRY_POINT_NOT_FOUND_IN_CONTRACT", KnownStarknetErrorCode::EntryPointNotFoundInContract),
        ("StarknetErrorCode.INSUFFICIENT_ACCOUNT_BALANCE", KnownStarknetErrorCode::InsufficientAccountBalance),
        ("StarknetErrorCode.INSUFFICIENT_MAX_FEE", KnownStarknetErrorCode::InsufficientMaxFee),
        ("StarknetErrorCode.INVALID_COMPILED_CLASS_HASH", KnownStarknetErrorCode::InvalidCompiledClassHash),
        ("StarknetErrorCode.INVALID_CONTRACT_CLASS_VERSION", KnownStarknetErrorCode::InvalidContractClassVersion),
        ("StarknetErrorCode.INVALID_TRANSACTION_NONCE", KnownStarknetErrorCode::InvalidTransactionNonce),
        ("StarknetErrorCode.INVALID_TRANSACTION_VERSION", KnownStarknetErrorCode::InvalidTransactionVersion),
        ("StarknetErrorCode.VALIDATE_FAILURE", KnownStarknetErrorCode::ValidateFailure),
        ("StarknetErrorCode.TRANSACTION_LIMIT_EXCEEDED", KnownStarknetErrorCode::TransactionLimitExceeded),
    ] {
        let starknet_error = deserialize_starknet_error(code_str, MESSAGE);
        let expected_starknet_error =
            StarknetError { code: StarknetErrorCode::KnownErrorCode(known_code), message: MESSAGE.to_string() };
...
fn unknown_error_code_deserialization() {
    ...
    let starknet_error = deserialize_starknet_error(CODE_STR, MESSAGE);
    let expected_starknet_error =
        StarknetError { code: StarknetErrorCode::UnknownErrorCode(CODE_STR.to_string()), message: MESSAGE.to_string() };
    assert_eq!(expected_starknet_error, starknet_error);
}
