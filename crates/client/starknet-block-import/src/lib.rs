...
//! transactions. Since the introduction of the safe intermediate representation (Sierra) for Cairo,
//! there's a possibility of the following attack:
//!   - User crafts a malicious Cairo contract the execution of which cannot be proven
//!   - User submits that transaction via a patched node that does not verify Sierra sources
//!   - The runtime does not have Sierra sources and therefore cannot check the validiy either
...
//!
//! Starknet block import addresses that issue as follows:
//!   - Upon receiving a new block it searches for declare transactions
//!   - For every declare transaction found, it tries to find according Sierra sources in the local
//!     DB
//!   - It tries to compile the source to check if they match the contract class from the
//!     transaction
...
//! In order to make it work in the multi-node setting, one needs to query missing Sierra sources
...
//! behind significantly it can fail to compile Sierra sources obtained from the recent compilers.
//! Similarly, old Sierra sources might not compile because of the broken backward compatibility.
