...
pub fn block_data_to_calldata(mut block_da_data: BlockDAData) -> Vec<U256> {
    ...
    let mut calldata: Vec<U256> = vec![
        U256::from_be_slice(&block_da_data.previous_state_root.0), // prev merkle root
        U256::from_be_slice(&block_da_data.new_state_root.0),      // new merkle root
        U256::from(block_da_data.block_number),                      // block number
        U256::from_be_slice(&block_da_data.block_hash.0.0),        // block hash
        U256::from_be_slice(&block_da_data.config_hash.0),         // config hash,
    ...
    for (addr, writes) in block_da_data.state_diff.storage_diffs {
        calldata.push(U256::from_be_slice(&addr.0.key().0));
        ...
        if let Some(class_hash) = class_flag {
            calldata.push(U256::from_be_slice(class_hash.0.bytes()));
        }

        for (key, val) in &writes {
            calldata.push(U256::from_be_slice(key.0.key().bytes()));
            calldata.push(U256::from_be_slice(val.bytes()));
        }
    }
    ...
    for (class_hash, compiled_class_hash) in &block_da_data.state_diff.declared_classes {
        calldata.push(U256::from_be_slice(class_hash.0.bytes()));
        calldata.push(U256::from_be_slice(compiled_class_hash.0.bytes()));
    }
...
pub fn da_word(class_flag: bool, nonce_change: Option<Nonce>, num_changes: u64) -> U256 {
    ...
    if let Some(new_nonce) = nonce_change {
        word += U256::from_be_slice(new_nonce.0.bytes()) + U256::from_str_radix(NONCE_BASE, 16).unwrap();
    }
