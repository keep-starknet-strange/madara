...
// use blockifier::abi::constants::L1_GAS_USAGE;
...
//     0, 131, 175, 211, 244, 202, 237, 198, 238, 191, 68, 36, 111, 229, 78, 56, 201, 94, 49, 121,
// 165, 236, 158, 168, 23,     64, 236, 165, 180, 130, 209, 46,
...
//         CoreResourcePrice { price_in_strk: item.price_in_strk, price_in_wei: item.price_in_wei as
// u64 }     }
// }
...
// ) -> TransactionExecutionResult<ResourcesMapping> { let state_changes_count =
...
//     let fee_transfer_call_info = execute_fee_transfer(state, block_context, account_tx_context,
// actual_fee)?;
...
//     let mut context = EntryPointExecutionContext::new(block_context.clone(), account_tx_context,
// max_steps);
...
// pub fn calculate_tx_fee(resources: &ResourcesMapping, block_context: &BlockContext) ->
// TransactionExecutionResult<Fee> {     let (l1_gas_usage, vm_resources) =
// extract_l1_gas_and_vm_usage(resources);     let l1_gas_by_vm_usage =
// calculate_l1_gas_by_vm_usage(block_context, &vm_resources)?;
...
//     let l1_gas_usage =
//         vm_resource_usage.remove(L1_GAS_USAGE).expect("`ResourcesMapping` does not have the key
// `l1_gas_usage`.");
...
//                 .ok_or(TransactionFeeError::ExecuteFeeTransferError());
