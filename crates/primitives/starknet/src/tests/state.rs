...
fn test_get_contract_class() {
    ...
    let contract_class = ContractClass::default(); // Replace with an actual ContractClass instance

    state.class_hash_to_class.insert(class_hash, contract_class.clone());

    let result = state.get_contract_class(&class_hash).unwrap();
...
fn test_get_contract_class_undeclared_class_hash() {
    ...
    let result = state.get_contract_class(&undeclared_class_hash);
