...
//             transaction_hash: Felt252Wrapper::TWO,
...
//         Transaction::Invoke(InvokeTransaction::V1(InvokeTransactionV1 {
//                 transaction_hash: Felt252Wrapper::ONE,
//                 max_fee: Felt252Wrapper::from_dec_str("1000").unwrap(),
//                 signature: signature.clone(),
...
//     let expected_hashes: Vec<Felt252Wrapper> = transactions.iter().map(|tx| tx.get_hash()).collect();
...
//     let vec_hashes: Vec<Felt252Wrapper> = transactions.iter().map(|tx| tx.get_hash()).collect();
