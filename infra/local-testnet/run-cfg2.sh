...
CMD_BOOTNODE="./target/release/madara \
	--chain=./crates/node/chain-specs/madara-local-cfg2-raw.json \
	--validator \
	--force-authoring \
	--rpc-cors=all \
	--rpc-external \
...
function start_boot_node(){
    ...
    echo "Starting $name"
    run_cmd="$CMD_BOOTNODE --node-key $node_key --port $port --rpc-port $rpc_port --base-path $base_path &> $log_file &"
