...
async fn get_transaction_receipt(
    ...
    // there is a delay between the sealing of the block and the transaction being available at the
    // client, hence sleeping for 100ms
...
async fn work_with_messages_to_l1(madara: &ThreadSafeMadaraClient) -> Result<(), anyhow::Error> {
    ...
    let deploy_tx_hash =
        assert_matches!(&txs[0], Ok(TransactionResult::Execution(rpc_response)) => rpc_response.transaction_hash);
