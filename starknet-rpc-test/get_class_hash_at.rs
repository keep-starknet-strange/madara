...
use madara_node_runner::constants::{TEST_CONTRACT_ADDRESS, TEST_CONTRACT_CLASS_HASH};
...
#[rstest]
#[tokio::test]
async fn work_ok_retrieving_class_hash(#[future] madara: MadaraClient) -> Result<(), anyhow::Error> {
    let madara = madara.await;
    let rpc = madara.get_starknet_client();
    let test_contract_address = FieldElement::from_hex_be(TEST_CONTRACT_ADDRESS).expect("Invalid Contract Address");
