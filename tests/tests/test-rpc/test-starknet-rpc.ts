...
describeDevMadara("Starknet RPC", (context) => {
  ...
  describe("pendingTransactions", async () => {
    it("should return an empty list", async () => {
      const transactions = await providerRPC.getPendingTransactions();
      expect(transactions).to.deep.equal([]);
    });

    it("should return pending transactions", async () => {
      const tx = await rpcTransfer(
        providerRPC,
        ARGENT_CONTRACT_NONCE,
        ARGENT_CONTRACT_ADDRESS,
        MINT_AMOUNT
      );
      const transaction = await providerRPC.getTransactionByHash(
        tx.transaction_hash
      );
      const transactions = await providerRPC.getPendingTransactions();
      expect(transactions).to.deep.equal({ transactions: [transaction] });
    });
  });
