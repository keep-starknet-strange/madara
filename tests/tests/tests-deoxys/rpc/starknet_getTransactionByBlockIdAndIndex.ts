...
async function checkDifferencesInBlocks() {
    ...
    for (let blockNumber = START_BLOCK; blockNumber < END_BLOCK; blockNumber++) {
        const differences = await benchmarkMethod('starknet_getTransactionByBlockIdAndIndex', [{"block_number": blockNumber}, 2]);
...
(async () => {
	...
	const singleCallDifferences = await benchmarkMethod('starknet_getTransactionByBlockIdAndIndex', [{"block_number": BLOCK_NUMBER}, 2]);
