...
const compareObjects = (remoteData: any, localData: any, path: string = ''): string => {
    let differences = '';
    ...
    for (const key in remoteData) {
        ...
        if (typeof remoteData[key] === 'object' && remoteData[key] !== null) {
            differences += compareObjects(remoteData[key], localData[key], currentPath);
...
async function benchmarkMethod(method: string, params: any[] = []): Promise<string> {
    const remoteResponse = await axios.post(REMOTE_RPC_URL, requestDataForMethod(method, params));
