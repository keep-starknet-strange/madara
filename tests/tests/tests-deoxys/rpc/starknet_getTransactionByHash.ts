...
async function benchmarkBlockAndTxs(blockNumber: number): Promise<string> {
  ...
  const txHashes: string[] = blockResponse.data.result.transactions || [];
  
  for (const txHash of txHashes) {
    console.log(`\x1b[36mTesting transaction: ${txHash}\x1b[0m`); // Logging the transaction being tested
    const differences = await benchmarkMethod("starknet_getTransactionByHash", [txHash]);
    if (differences.includes("\x1b[31mDIFFERENCE")) {
      console.log(`\x1b[31mTransaction ${txHash} has differences.\x1b[0m`);
    } else {
      console.log(`\x1b[32mTransaction ${txHash} matches.\x1b[0m`);
    }
...
async function checkDifferencesInBlocks() {
  ...
  for (let blockNumber = START_BLOCK; blockNumber < END_BLOCK; blockNumber++) {
    const differences = await benchmarkBlockAndTxs(blockNumber);
    if (differences.includes("\x1b[31mDIFFERENCE")) {
