...
async function checkDifferencesInBlocks() {
    ...
    for (const tx of TRANSACTIONS) {
        const differences = await benchmarkMethod('starknet_getTransactionReceipt', [tx]);
    ...
    if (blocksWithDifferences.length === 0) {
        ...
        console.log("\x1b[31mDifferences found in transactions:\x1b[0m", blocksWithDifferences);
    }
}
...
(async () => {
	...
	const singleCallDifferences = await benchmarkMethod('starknet_getTransactionReceipt', ["0x3c28efbc632f0ece25dc30aa2c5035f9aa907e43a42103609b9aded29fde516"]);
