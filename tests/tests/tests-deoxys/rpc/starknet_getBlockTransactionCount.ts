...
async function checkDifferencesInBlocks() {
    ...
    for (let blockNumber = 0; blockNumber < FULL_BLOCK; blockNumber++) {
        const differences = await benchmarkMethod('starknet_getBlockTransactionCount', [{"block_number": blockNumber}]);
...
(async () => {
	...
	const singleCallDifferences = await benchmarkMethod('starknet_getBlockTransactionCount', [{"block_number": BLOCK_NUMBER}]);
