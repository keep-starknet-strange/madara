...
async function checkDifferencesInBlocks() {
    ...
    for (let blockNumber = START_BLOCK; blockNumber < END_BLOCK; blockNumber++) {
        const differences = await benchmarkMethod('starknet_getStateUpdate', [{"block_number": blockNumber}]);
...
(async () => {
	...
	const singleCallDifferences = await benchmarkMethod('starknet_getStateUpdate', [{"block_number": BLOCK_NUMBER}]);
