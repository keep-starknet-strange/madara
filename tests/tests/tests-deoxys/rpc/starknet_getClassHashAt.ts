...
async function checkDifferencesInBlocks() {
    ...
    for (let blockNumber = START_BLOCK; blockNumber < END_BLOCK; blockNumber++) {
        const differences = await benchmarkMethod('starknet_getClassHashAt', [LATEST, LATEST]);
...
(async () => {
	...
	const singleCallDifferences = await benchmarkMethod('starknet_getClassHashAt', [LATEST, LATEST]);
