...
const getBlockDetails = async (
  ...
  const fees = await Promise.all(
    block.extrinsics.map((ext) =>
      api.rpc.payment.queryInfo(ext.toHex(), block.header.parentHash)
    )
  );
...
// TODO: Fix with Starknet fee model
...
//   const fromPreBlockHash = (
//     await api.rpc.chain.getBlockHash(fromBlockNumber - 1)
//   ).toString();
...
//   const toBlockHash = (
//     await api.rpc.chain.getBlockHash(toBlockNumber)
//   ).toString();
//   const toSupply = (await (
//     await api.at(toBlockHash)
//   ).query.balances.totalIssuance()) as any;
...
//                 let feePortions = calculateFeePortions(
...
//                     toBalance.data.free.toBigInt()) as any) -
//                   expectedBalanceDiff
//                 ).toString()
//               );
//             }
...
export async function jumpRounds(
  context: DevTestContext,
  count: Number
): Promise<string | null> {
  const round = (
...
export const checkTimeSliceForUpgrades = async (
  ...
  const onChainRt = (await apiAt.query.system.lastRuntimeUpgrade()).unwrap()
    .specVersion;
