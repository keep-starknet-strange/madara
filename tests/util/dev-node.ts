...
export type RuntimeChain = "madara";

// Stores if the node has already started.
// It is used when a test file contains multiple describeDevMadara. Those are
...
export async function startMadaraDevNode(
  ...
  runningNode.once("exit", () => {
    ...
    nodeStarted = false;
    debug(
      `Exiting dev node: --port=${p2pPort} --rpc-port=${rpcPort} --ws-port=${wsPort}`
    );
  });
