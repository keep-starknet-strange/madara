...
exports.extractInfo =
  exports.isExtrinsicSuccessful =
  exports.extractError =
  exports.getDispatchError =
  exports.filterAndApply =
  exports.createBlockWithExtrinsicParachain =
  exports.logEvents =
  exports.waitOneBlock =
    void 0;
...
async function logEvents(api, name) {
  api.derive.chain.subscribeNewHeads(async (header) => {
    ...
    const allRecords = await (await api.at(header.hash)).query.system.events();
    allRecords.forEach((e, i) => {
      debug(
        `${name} Event :`,
        i,
        header.hash.toHex(),
        e.toHuman().event.section,
        e.toHuman().event.method,
      );
    });
  });
}
